...
// Function to add Gaussian noise to a value (used for x, y, or angle)
...
void addNoiseToXYZAndAngleInDat(const string &inputDatFile, const string &outputDatFile, double noise_percentage)
{
    ...
    while (inFile)
    {
        ...
        // Extract y-coordinate (next 2 bytes)
        int y = unpackXY(buffer[2], buffer[3]);

        // Add noise to y-coordinate
        y = add_noise(y, noise_percentage);

        // Repack the y-coordinate, preserving the top 2 bits (minutiae type/flags)
        packXY(y, buffer[2], buffer[3], buffer[2]);

        // Add noise to angle (5th byte in the buffer)
        buffer[4] = add_noise_to_angle(buffer[4], noise_percentage);

        // Write the modified buffer (with noise in x, y, and angle) to the output file
        outFile.write(reinterpret_cast<char *>(buffer), sizeof(buffer));
    }

    inFile.close();
    outFile.close();
    cout << "Noise added to X, Y, and Angle at " << noise_percentage * 100 << "% and saved to " << outputDatFile << endl;
}
...
int main()
{
    ...
    for (double noise_percentage : noise_percentages)
    {
        ...
        string outputDatFile = "fmd_with_xyz_and_angle_noise_" + to_string(static_cast<int>(noise_percentage * 100)) + ".dat";

        // Apply noise and save to file
        addNoiseToXYZAndAngleInDat(inputDatFile, outputDatFile, noise_percentage);
    }
